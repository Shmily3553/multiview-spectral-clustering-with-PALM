...
%% load data
% 3-sources
load data\3-sources.mat bbc guardian reuters truth;
view = 3;
cluster = 6;
G{1,1} = bbc;
G{1,2} = guardian;
...
save('result\best_3sources.mat', "best");
...
%     disp('closed-form solution')
%     for j = 1:itr
%         [G, G_star] = closedform(view, lambda, theta(l), G, G_star, L, I, A);
%     end
%     G_cf = G_star;
%     cm_cf = measure(G_cf, cluster, truth);
%     cms_cf = [cms_cf; cm_cf];
...
%     disp('gradient descent solution')
%     for j = 1:itr 
%         [G, G_star] = gradientdescent(view, lambda, theta(l), G, G_star, L, A);
%     end
%     G_gd = G_star;
%     cm_gd = measure(G_gd, cluster, truth);
%     cms_gd = [cms_gd; cm_gd];
...
% best_gd = max(cms_gd);
% 
% save('result\best_cf_theta_3sources.mat', "best_cf");
% save('result\best_gd_theta_3sources.mat', "best_gd");
